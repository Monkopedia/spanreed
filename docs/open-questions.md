...
- **Codex on the bus** (answered empirically, 2026-09-14, and the answer is *not yet*): a Codex TUI session cannot be reached by another process on `codex-cli 0.154.0`. Its ipc socket is stale, nothing advertises a server, and and a second `app-server` cannot enumerate or create threads — `thread/list` and `thread/start` time out. (An earlier version of this entry blamed lock contention with the TUI; that was retracted when the same timeouts occurred with zero codex processes running. The cause is open; startup network work is the current candidate.) So a Codex agent could receive mail but could not be *woken* by it, making it a mailbox rather than a peer. See `experiments/codex-app-server-spike/` for the eight-run record and a working WebSocket client. What would change it: OpenAI shipping an external-wake primitive (four open issues), or driving Codex through the App/VS Code extension, which do register as thread owners. Not worth building against until one of those holds.
