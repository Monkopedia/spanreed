...
- **Sender authentication** (resolved 2026-08-18): `send_message`'s `from_agent` is caller-asserted and verified nowhere. Asked directly, the owner ruled *"no, if it's on the bus you can trust it"* — the bus is a trusted channel and this is accepted design, not a gap. Deriving the sender from the calling process was measured and rejected on cost as well: 31% of bus traffic (1590 messages, 47 senders — cron jobs, the webhook receiver, triage and reviewer subagents) has no session to derive an identity from, so it would break the fleet's automation layer. Validating `from_agent` against the registry fails the same way *and* does not stop impersonation. See `Monkopedia/spanreed#16`. Distinct from, and not resolved by this: an id can be wrong by *accident* — see `#23`.
