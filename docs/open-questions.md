...
- **Codex on the bus** (answered empirically, 2026-09-14, and the answer is *not yet*): a Codex TUI session cannot be reached by another process on `codex-cli 0.154.0`. Its ipc socket is stale, nothing advertises a server, and a second `app-server` is locked out of the thread store while the TUI holds it — `thread/list` returns instantly with no TUI running and times out with one. So a Codex agent could receive mail but could not be *woken* by it, making it a mailbox rather than a peer. See `experiments/codex-app-server-spike/` for the eight-run record and a working WebSocket client. What would change it: OpenAI shipping an external-wake primitive (four open issues), or driving Codex through the App/VS Code extension, which do register as thread owners. Not worth building against until one of those holds.
