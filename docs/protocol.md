...
**One writer nevertheless uses `getppid()` on purpose, and the reasoning inverts there.** `_ensure_registered` creates a stub when `focus`/`status`/`name` find no entry. It is reachable when identity resolution has already fallen back to the cwd — so *on that path* the id it is about to create is by construction **not** that session's.

That is not the only way in, and the exception is written for the dangerous one. `SPANREED_AGENT_NAME` is precedence 1 in resolution and never consults the registry, so an override session reaches this stub for **its own** id with no fallback involved. The stub then records an ephemeral pid, so that session never appears in `list_agents` and can never be found by the anchor. Nothing here regressed it and this section does not fix it — see #38, which owns whether the stub should special-case the override. Recording the anchor there would attach the session's liveness to a foreign entry permanently: it would never decay, `is_stale` would confirm it, and resolution would then find two live entries claiming one pid. The ephemeral shell pid is the correct choice precisely because it **fails closed** — the stub evaporates when the command does. The rule is not "prefer the anchor"; it is *record the pid of the process whose liveness this entry tracks*, and for a stub for someone else that is emphatically not us.
