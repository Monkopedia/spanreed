...
So the CLI's writers read `$CLAUDE_PID` and fall back to `getppid()` only when it is unset. **`getppid()` is not a substitute for the anchor inside a session** — under a Bash tool the parent is an ephemeral `zsh`, alive for the length of one command, and an entry written that way is not a correct value that decays but *the wrong value at the moment of writing*.

**One writer nevertheless uses `getppid()` on purpose, and the reasoning inverts there.** `_ensure_registered` creates a stub when `focus`/`status`/`name` find no entry. It is reachable only when identity resolution has *already* fallen back to the cwd — so inside a session, the id it is about to create is by construction **not** that session's. Recording the anchor there would attach the session's liveness to a foreign entry permanently: it would never decay, `is_stale` would confirm it, and resolution would then find two live entries claiming one pid. The ephemeral shell pid is the correct choice precisely because it **fails closed** — the stub evaporates when the command does. The rule is not "prefer the anchor"; it is *record the pid of the process whose liveness this entry tracks*, and for a stub for someone else that is emphatically not us. Its symptom is that the agent silently disappears from `list_agents` while running normally, and that identity resolution above falls back to the cwd with no warning, because a wrong pid is indistinguishable from an unregistered session.
